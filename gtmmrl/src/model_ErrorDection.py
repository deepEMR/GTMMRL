...
class GTXModelOutput(ModelOutput):
    ...
    Args:
        ...
        cross_encoder_attentions (:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            weighted average in the self-attention heads.
    """
    loss_bce: [torch.FloatTensor] = None
...
class GTXModel(GTXPreTrainedModel):
    def __init__(self, config):
        ...
        self.token_classifier_m = nn.Linear(config.hidden_size, 1)
        self.multilabel_classifier_m = nn.Sequential(nn.Linear(config.hidden_size, config.hidden_size),
                                                   nn.ReLU(),
                                                   nn.Linear(config.hidden_size, config.num_labels))
        self.model_pairs = [[self.graph_encoder, self.graph_encoder_m],
                            ...
                            [self.token_classifier, self.token_classifier_m],
                            [self.multilabel_classifier, self.multilabel_classifier_m],
    ...
    def forward(
        ...
        if kg_label is not None:
            if kg_input_ids.size(-1) == kg_label.size(-1):
                _size = cross_output_pos.kg_feats.shape[:-1]
                score = self.token_classifier(cross_output_pos.kg_feats.view(-1, self.config.hidden_size)).view(_size)
                bce_loss = self.loss_fcts["bce"](score, kg_label)

                if self.config.MoDDownstream:
                    with torch.no_grad():
                        score_m = self.token_classifier_m(output_cross_m.kg_feats.view(-1, self.config.hidden_size)).view(
                            _size)
                    kg_label_soft = F.softmax(score_m, dim=-1)
                    # bce_loss_disstill = self.loss_fcts["bce"](score, kg_label_soft)
                    # loss_distill = bce_loss_disstill.view(-1).mean()
                    bce_loss_disstill = -torch.sum(F.log_softmax(score, dim=-1) * kg_label_soft, dim=-1)
                    loss_distill = bce_loss_disstill.view(-1).mean()
                    bce_loss = (1 - alpha) * bce_loss + alpha * loss_distill
            else:
                score = self.multilabel_classifier(cross_output_pos.kg_feats[:, 0])
                bce_loss = self.loss_fcts["bce"](score, kg_label)

                if self.config.MoDDownstream:
                    with torch.no_grad():
                        score_m = self.multilabel_classifier_m(output_cross_m.kg_feats[:,0])
                        kg_label_soft = F.softmax(score_m, dim=-1)
                    bce_loss_disstill = self.loss_fcts["bce"](score, kg_label_soft)
                    loss_distill = bce_loss_disstill.view(-1).mean()
                    bce_loss = (1 - alpha) * bce_loss + alpha * loss_distill
            ...
            _size = cross_output_pos.lang_feats.shape[:-1]
            score = self.token_classifier(cross_output_pos.lang_feats.view(-1, self.config.hidden_size)).view(_size)
            bce_loss = self.loss_fcts["bce"](score, lm_label)
            focal_weight = (score.sigmoid() - lm_label).square().abs().detach().clone()
            bce_loss = (bce_loss * focal_weight).mean()

            if self.config.MoDDownstream:
                with torch.no_grad():
                    score_m = self.token_classifier_m(
                        output_cross_m.lang_feats.view(-1, self.config.hidden_size)).view(_size)
                    lm_label_soft = F.softmax(output_cross_m, dim=-1)
                focal_weight_soft = (score_m.sigmoid() - lm_label_soft).square().abs().detach().clone()
                bce_loss_distill = self.loss_fcts["bce"](score, lm_label_soft)
                focal_weight_distill = (score_m.sigmoid() - lm_label_soft).square().abs().detach().clone()
                bce_loss_distill = (bce_loss_distill * focal_weight).mean()
                bce_loss = (1 - alpha) * bce_loss + alpha * bce_loss_distill
        else:
            bce_loss = None
        ...
        if not return_dict:
            return (lang_output, kg_output, score) + hidden_states + all_attentions

        return GTXModelOutput(
            loss_bce=bce_loss,
            loss_ita=None,
            loss_mlm=loss_mlm,
            language_prediction_scores=None,
            pooled_output=score,
...
class MedModelForErrorDetection(GTXPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
    ...
    def get_out_put(
        ...
        loss_dict['loss'] = GTX_output.loss_bce.mean().detach()
        total_loss += GTX_output.loss_bce
