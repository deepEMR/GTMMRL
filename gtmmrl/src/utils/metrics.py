...
def bleu_all(list_of_references, hypotheses):
    ...
    chencherry = SmoothingFunction()
    bleus = corpus_bleu(list_of_references=list_of_references, hypotheses=hypotheses,
...
def metrics_for_tasks(task,
    ...
    if task == "Pre":
        if batch['lm_label'] is not None:
            lm_pred = torch.max(outputs.lang_prediction_logits, dim=2)[-1][:batch['lm_label'].size(0)][
                ~batch['lm_label'].eq(-100)].view(-1).long()
            lm_gt = batch['lm_label'][~batch['lm_label'].eq(-100)].view(-1).long()
            metrics[f"{stage}_lm_acc"] = lm_pred == lm_gt
        if batch['kg_label'] is not None:
            kg_pred = torch.max(outputs.kg_prediction_logits, dim=2)[-1][:batch['lm_label'].size(0)][
                ~batch['kg_label'].eq(-100)].view(-1).long()
        ...
        pred = torch.sigmoid(outputs.pooled_logits)
        gt = batch['label']
        ...
        pred = torch.sigmoid(outputs[:, living_gt_idx]).cpu()
        pred_ind = pred > 0.5
        metrics[f"{stage}_acc"] = accuracy_score(gt, pred_ind)
        ...
        if stage == "valid":
            ...
            batch_gt_strings = tokenizer.batch_decode(org_lang_input_ids, skip_special_tokens=True)
            batch_pred_strings = tokenizer.batch_decode(pred, skip_special_tokens=True)
            # print('batch_pred_strings>>>>',batch_pred_strings)
            rouge_tot_scores = rouge_all(references=batch_gt_strings, hypotheses=batch_pred_strings)
            for rouge_metric in ['rouge-1', 'rouge-2', 'rouge-l']:
                for rouge_sub_metric in ['f', 'p', 'r']:
                    metrics[f"{stage}_{rouge_metric}_{rouge_sub_metric}"] = torch.tensor(
                        rouge_tot_scores[rouge_metric][rouge_sub_metric])
