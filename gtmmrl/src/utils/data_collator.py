        ...
        for k, v in first.items():
            ...
            if v is not None:
                if (k == "kg_attention_mask") and not isinstance(v, str):
                    ...
                    # print(k,' --> ',v.__len__(), ' --> ', [features[i].items().__len__() for i in range(features.__len__())])
                    # print(k)
                    if isinstance(v, torch.Tensor):
                        batch[k] = torch.stack([f[k] for f in features])
                    else:
                        ...
                        #     batch[k] = torch.tensor([f[k] for f in features])
                        # else:
                        #     batch[k] = torch.tensor([f[k][0:769] for f in features])
        ...
        for k, v in batch.items():
            if v is not None:
                if ('rc' in k) or ('label' in k):
                    ...
                    batch[k] = torch.cat([batch[k].detach().clone()[(torch.arange(batch_size) + idx) % batch_size]
                                          for idx in range(self.n_negatives+1)],dim=0)
...
class ErrorDetection_DataCollator:
    ...
    def __call__(self,features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        ...
        #     notifier.critical("Fusion layer activated")
        #     batch = self.tensorize_ext_know(batch)
        #     notifier.warning(batch['kg_ext_input_ids'][0])
        #     raise ValueError()
        batch['kg_label'] = batch['label'].float()
        batch.pop('label')
    ...
    # def tensorize_ext_know(self, batch):
    #     inputs = batch['kg_input_ids']
    #     num_batch = inputs.size(0)
    #     kg_ext_input_ids = list()
    #     kg_ext_attention_mask = list()
    #     for b in range(num_batch):
    #         desc_ids = list(itertools.chain(*[self.id2desc[k] if k in self.id2desc else [] for k in inputs[b].tolist()]))
    #         padded_desc_ids = desc_ids + [self.tokenizer.pad_token_id]*(1024 -len(desc_ids))
    #         kg_ext_input_ids.append(padded_desc_ids)
    #         padding_mask = [0 if _ids==self.tokenizer.pad_token_id else 1 for _ids in padded_desc_ids]
    #         kg_ext_attention_mask.append(padding_mask)
    #     batch['kg_ext_input_ids'] = torch.tensor(kg_ext_input_ids)
    #     batch['kg_ext_attention_mask'] = torch.tensor(kg_ext_attention_mask)
