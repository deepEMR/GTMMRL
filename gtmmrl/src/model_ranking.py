...
class GTXModel(GTXPreTrainedModel):
    ...
    def forward(
        ...
        # if self.config.MoDDownstream and self.config.hard:
        with torch.no_grad():
        ...
        elif not self.config.hard:

            pooled_output_all = self.itm_head(cross_output_pos.kg_feats, cross_output_pos.lang_feats)
            loss_gtm = F.cross_entropy(pooled_output_all.float(), label)

            # negative_sampling
            # batch_size = kg_feats.size(0)
            # lang_feats_gtm = torch.cat([lang_feats.detach().clone()[(torch.arange(batch_size) + idx) % batch_size] for idx in range(2)], dim=0)
            # language_hidden_states_gtm = torch.cat([language_hidden_states.detach().clone()[(torch.arange(batch_size) + idx) % batch_size] for idx in range(2)], dim=0)
            # lang_attention_mask_gtm = torch.cat([lang_attention_mask.detach().clone()[(torch.arange(batch_size) + idx) % batch_size] for idx in range(2)], dim=0)
            #
            #
            # kg_feats_gtm = torch.cat([kg_feats.detach().clone() for _ in range(2)], dim=0)
            # kg_hidden_states_gtm = torch.cat([kg_hidden_states.detach().clone() for _ in range(2)], dim=0)
...
class MedModelForRanking(GTXPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
    ...
    def forward(
        ...
        if self.config.GTA and self.config.MoDDownstream and self.config.hard:
    ...
    def get_out_put(
        ...
        if label is not None:
            ...
            total_loss = F.cross_entropy(cross_relationship_score, label)
            loss_dict['loss'] = total_loss.mean().detach()
        else:
            total_loss = None
