...
class GTXModelOutput(ModelOutput):
    ...
    Args:
        ...
        cross_encoder_attentions (:obj:`tuple(torch.FloatTensor)`, `optional`, returned when ``output_attentions=True`` is passed or when ``config.output_attentions=True``):
            ...
            weighted average in the self-attention heads.
    """
    tempora_loss: [torch.FloatTensor] = None
...
class MGCLPooler(nn.Module):
    def __init__(self, config):
        super(MGCLPooler, self).__init__()
...
class GTXModel(GTXPreTrainedModel):
    def __init__(self, config):
        ...
        self.pooler_m = MGCLPooler(config)
    ...
    def forward(
        ...
        cross_output_pos = copy.copy(output_cross)

        ##================= loss_tempora for Death30 and Readmission Task  ====================##
        multi_label_score = self.pooler(cross_output_pos.kg_feats, cross_output_pos.lang_feats)
        tempora_loss = self.loss_fcts["bce"](multi_label_score.squeeze(dim=1), label.float())

        if self.config.MoDDownstream:
            with torch.no_grad():
                tempora_score_soft = self.pooler_m(output_cross_m.kg_feats, output_cross_m.lang_feats)
                tempora_label_soft = F.softmax(tempora_score_soft, dim=-1)
                loss_distill_multi_label = -torch.sum(F.log_softmax(multi_label_score, dim=-1) * tempora_label_soft,
                                                      dim=-1)
                loss_distill = loss_distill_multi_label.view(-1).mean()
            tempora_loss = (1 - alpha) * tempora_loss + alpha * loss_distill
        ...
        pooled_output = multi_label_score
    ...
    def get_gtx_embeddings(
        ...
        cross_output_tempora = copy.copy(cross_output_pos)
        multi_label_score = self.pooler(cross_output_tempora.kg_feats, cross_output_tempora.lang_feats)
        tempora_loss = self.loss_fcts["bce"](multi_label_score.squeeze(dim=1), label.float())
...
class MedModelForTemporalPred(GTXPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # Configuration
        self.config = config
    ...
    def forward(
        ...
        loss_dict['loss_tempora'] = GTX_output.tempora_loss.mean().detach()
        total_loss += GTX_output.tempora_loss
    ...
    def get_out_put(
        ...
        # multi_label_score = self.pooler(kg_output, lang_output)
